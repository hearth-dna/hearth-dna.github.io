/// The database schema, copied verbatim from `frontend/src/db/schema.ts` (ADR 0010). The web app is
/// the reference: change it there first, then paste the new text here. `SchemaTests` compares the
/// two, so a copy that drifts fails the build's tests instead of writing rows the others cannot read.
let SCHEMA_SQL = """
